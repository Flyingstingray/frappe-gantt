const v = "year", k = "month", M = "day", D = "hour", Y = "minute", T = "second", E = "millisecond", d = {
  ...
  diff(n, t, e = "day") {
    ...
    if (h = _ * 12 + g, n.getDate() < t.getDate()) {
      h -= 1;
      let c = new Date(n.getFullYear(), n.getMonth(), 0).getDate(), p = c - t.getDate() + n.getDate();
      h += p / c;
    } else {
      let c = n.getDate() - t.getDate(), p = new Date(n.getFullYear(), n.getMonth() + 1, 0).getDate();
      h += c / p;
    }
...
class q {
  ...
  calculate_path() {
    let t;
    (this.type === "FS" || this.type === "FF") && (t = this.from_task.$bar.getX() + this.from_task.$bar.getWidth()), (this.type === "SS" || this.type === "SF") && (t = this.from_task.$bar.getX() + this.from_task.$bar.getWidth() / 10);
...
const b = [
  ...
  popup: (n) => {
    ...
    n.set_details(
      `${t} - ${e} (${z(n.task._start, n.task._end)} business days${n.task.ignored_duration ? " + " + n.task.ignored_duration + " excluded" : ""})`
    );
  },
  popup_on: "click",
  readonly_progress: !1,
